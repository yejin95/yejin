...
if ($w == "")
{
    ...
    if ($member[mb_level] < $board[bo_write_level]) {
        if ($member[mb_id])
			...
			if ($bo_table == 'counsel') {
				alert("전문의상담 문의글 작성은 로그인 이후 가능합니다.", "./login.php?$qstr&url=".urlencode("$_SERVER[PHP_SELF]?bo_table=$bo_table"));
			}
			else {
...
# 모바일일 경우 에디터 비활성화
if ($g4['view_type'] != '') $is_dhtml_editor = false;
...
if(isset($_GET['qa'])){
    $qa = "{$_GET['qa']} 에 대해서 상담신청 해주셨습니다.";
    include_once ("$board_skin_path/write.skin_0806.php");
}else{
