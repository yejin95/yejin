...
if ($is_captcha) {
    if ($w=='' || $w=='r') {
        if ($recaptcha['use']) {
            require "{$g4['path']}/lib/ReCaptcha/autoload.php";
            $recaptcha_obj = new \ReCaptcha\ReCaptcha($recaptcha['secret_key']);
            $resp = $recaptcha_obj->verify($_POST['g-recaptcha-response'], $_SERVER['REMOTE_ADDR']);
            if ($resp->isSuccess()) {
                // verified!
            } else {
                $errors = $resp->getErrorCodes();
                $tmp = "자동등록방지코드가 올바르지 않습니다.\\r\\n";
			...
			if ($bo_table == 'N14_2' || $bo_table == 'N15_1' || $bo_table == 'event' || $bo_table == 'landingUS' || $bo_table == 'landingJP') 
				;
			else {
...
if ($w == "" || $w == "r") 
{
    if ($member[mb_id]) 
    {
		...
		if (isset($_POST['wr_name']) && !empty($_POST['wr_name'])) $wr_name = $_POST['wr_name'];
		if (isset($_POST['wr_email']) && !empty($_POST['wr_email'])) $wr_email = $_POST['wr_email'];
    } 
    else 
    {
        $mb_id = "";
		...
		if (!$wr_password) $wr_password = time();
...
else if ($w == "u") 
{
	...
	for ($i=1; $i<10; $i++) 
		if (!isset($_POST['wr_'.$i])) $GLOBALS['wr_'.$i] = $write['wr_'.$i];
...
/* 접속경로 추적을 없애고 wr_9 에 referer 를 넣는다 */
$row = sql_fetch("SELECT * FROM {$g4['visit_table']} WHERE vi_id = '{$_SESSION['vi_id']}' ");
if ($row['vi_id']) {
	sql_query("UPDATE {$write_table} SET wr_9 = '{$row['vi_referer']}' WHERE wr_id = '{$wr_id}' ");
}
