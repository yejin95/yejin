<?php
if ($_SERVER['REMOTE_ADDR'] != '1.177.28.24' && $_SERVER['REMOTE_ADDR'] != '180.182.79.76') {
	#die('working');
}
if ($_SERVER['HTTPS'] != 'on') {
	if (strpos($_SERVER['PHP_SELF'], '/adm') === false && strpos($_SERVER['PHP_SELF'], '/cheditor') === false) {
		#header("Location: https://.kr{$_SERVER['REQUEST_URI']}", 302); exit;
	}
}
if ($_SERVER['HTTP_HOST'] != 'www.newstyleclinic.com') {
	#header("Location: http://www.newstyleclinic.com{$_SERVER['REQUEST_URI']}", 302); exit;
}
/*******************************************************************************
** 공통 변수, 상수, 코드
*******************************************************************************/
...
if ($_SESSION['ss_mb_id']) // 로그인중이라면
{
    ...
    if (substr($member['mb_today_login'], 0, 10) != $g4['time_ymd'])
    {
		...
		// mb_2 휴면대상회원메일발송일 mb_3 휴면회원처리일자
        $sql = " update {$g4['member_table']} set mb_today_login = '{$g4['time_ymdhis']}', mb_login_ip = '{$_SERVER['REMOTE_ADDR']}', mb_2 = '', mb_3 = '' where mb_id = '{$member['mb_id']}' ";
		sql_query($sql);
    }
}
else
...
// 회원, 비회원 구분
$is_member = $is_guest = $is_captcha = false;
if ($member['mb_id'])
    $is_member = true;
else
    $is_guest = $is_captcha = true;

if ($member['mb_level'] < 3) {
	if (in_array($bo_table, array('counsel'))) $is_captcha = true;
}
